...
module.exports = class PetController {
    ...
    static async updatePetById(req, res) {
        ...
        if (!weight) {
            ...
            udpatedData.weight = weight
        }
        if (!color) {
            res.status(422).json({ message: "A cor é obrigatória!" })
            return
        } else {
            udpatedData.color = color
        }
        ...
        if (images.length === 0) {
            ...
            udpatedData.images = []
            images.map((image) => {
                udpatedData.images.push(image.filename)
            })
        }

        await Pet.findByIdAndUpdate(id, udpatedData)
    ...
    static async schedule(req, res) {
        ...
        res.status(200).json({
            message: `A visita foi agendada com sucesso, entre em com ${pet.user.name} pelo telefone ${pet.user.phone}`
        })
